...
class SectorDB:
    ...
    def _do_dynamic_load(self, on_complete):
        try:
                ...
                try:
                    ...
                    if time.time() - mtime < CACHE_TTL:
                        ...
                        if cache.get('version') == CACHE_VERSION:
                            ...
                            listing_levels = cache.get('listing_levels', {})
                            cache_valid = True
                except Exception:
                    pass

            if not cache_valid:
                from .sector_index import fetch_all_moex_tickers, fetch_all_sector_tickers_parallel, fetch_listing_levels
                all_tickers = fetch_all_moex_tickers()
                moex_map = fetch_all_sector_tickers_parallel()
                listing_levels = fetch_listing_levels()
                try:
            ...
            self._listing_levels = listing_levels
    ...
    def get_listing_level(self, ticker):
        return self._listing_levels.get(ticker.upper())
