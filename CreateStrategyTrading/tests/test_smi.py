...
from strategy.smi import check_smi
...
class TestSmi(unittest.TestCase):
    def test_no_signal_out_of_range(self):
        self.assertIsNone(check_smi([make_candle(100, 101, 102, 99)], 5, [100], 2.0))

    def test_no_signal_far_from_level(self):
        candles = [make_candle(100, 101, 102, 99) for _ in range(80)]
        self.assertIsNone(check_smi(candles, len(candles) - 1, [150], 2.0))

    def test_buy_signal(self):
        candles = [make_candle(120, 121, 122, 119) for _ in range(20)]
        candles += [make_candle(115, 112, 117, 111) for _ in range(15)]
        candles += [make_candle(100, 98, 102, 97) for _ in range(15)]
        candles += [make_candle(98, 100, 102, 97)]
        levels = [100]
        atr = 2.0
        signal = check_smi(candles, len(candles) - 1, levels, atr, atr_sl=1.0, atr_tp=2.0, level_proximity=2.0)
        if signal is None:
    ...
    def test_sell_signal(self):
        candles = [make_candle(80, 81, 82, 79) for _ in range(20)]
        candles += [make_candle(85, 88, 89, 84) for _ in range(15)]
