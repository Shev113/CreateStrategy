...
from strategy.dinapoli import check_dinapoli
...
class TestDinapoli(unittest.TestCase):
    def test_no_signal_out_of_range(self):
        candles = [make_candle(100, 101, 102, 99)]
        self.assertIsNone(check_dinapoli(candles, 5, [100], 2.0))

    def test_no_signal_far_from_level(self):
        candles = [make_candle(100, 101, 102, 99) for _ in range(110)]
        self.assertIsNone(check_dinapoli(candles, 109, [200], 2.0))

    def test_buy_signal(self):
        candles = [make_candle(100, 101, 102, 99) for _ in range(30)]
        candles += [make_candle(100 + i, 101 + i, 102 + i, 99 + i) for i in range(70)]
        signal = check_dinapoli(candles, 99, [170], 2.0, atr_sl=1.0, atr_tp=2.0, dinapoli_k=8, dinapoli_d=3, dinapoli_slow=3, dinapoli_ob=70, dinapoli_os=30, level_proximity=2.0)
        if signal is None:
    ...
    def test_sell_signal(self):
        ...
        signal = check_dinapoli(candles, 99, [130], 2.0, atr_sl=1.0, atr_tp=2.0, dinapoli_k=8, dinapoli_d=3, dinapoli_slow=3, dinapoli_ob=70, dinapoli_os=30, level_proximity=2.0)
        if signal is None:
            self.skipTest("signal not triggered with synthetic data")
